...
export default function LetsCreate() {
  ...
  useEffect(() => {
    ...
    mm.add("(min-width: 768px) and (prefers-reduced-motion: no-preference)", () => {
      ...
      // grow eases IN gently (power2.inOut) so it's not snappy at the start —
      // the bubble swells up, holds, then sinks back into the dark.
      tl
        .to("[data-lc-bubble]", { scale: 1.4, duration: 1.25, ease: "power2.inOut" }, 0)
        .to("[data-lc-content]", { autoAlpha: 1, scale: 1, duration: 0.8, ease: "power2.out" }, 0.5)
        .to("[data-lc-content]", { autoAlpha: 1, duration: 0.55 }, 1.3)
        .to("[data-lc-content]", { autoAlpha: 0, scale: 0.94, duration: 0.4, ease: "power2.in" }, 1.75)
        .to("[data-lc-bubble]", { scale: 0, duration: 0.85, ease: "power2.inOut" }, 1.85);
    });
  ...
  return (
    <section ref={root} data-theme="dark" data-surface="media" className="relative min-h-screen bg-[#050505] md:min-h-[150vh]">
