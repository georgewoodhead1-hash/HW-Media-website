...
// 05/06 — TESTIMONIALS then FAQS, both in PLAIN FLOW (George: "one smooth
// scroll like 1820" — no pins, the page never stops). The blend is the
// overlap of passage animations: the testimonial ledger dissipates as it
// leaves the viewport while the FAQ ledger builds row by row as it enters.
// The FAQ's bottom line never fades — it rides down the page into the
// Defender band, where the pen line picks it up.
...
export default function TestimonialsFaqs() {
  ...
  // ── TESTIMONIALS: rise in on passage, dissipate on the way out ──
  useEffect(() => {
  ...
  // ── FAQS: rows build ONE BY ONE on their own passage; dissipate on exit,
  //    except the bottom line which rides on into the band ──
  useEffect(() => {
  ...
  return (
    <>
      {/* ── TESTIMONIALS — plain flow, no pin ── */}
      <section
        ref={testiRef}
        data-theme="dark"
        data-surface="page"
      ...
      >
        {/* header (George): big bracketed title like Trusted by, then the
            hairline WITH the plusses drawing underneath it */}
      ...
      {/* ── FAQS — plain flow, rows building on their own passage ── */}
      <section
        ref={faqRef}
        data-theme="dark"
        data-surface="page"
      ...
      >
        {/* header (George): just the big bracketed title, NO side lines —
